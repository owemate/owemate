...
function AppContent() {
  ...
  if(app.screen==='reminders')return <AppShell><ReminderSettingsScreen enabled={app.remindersEnabled} onEnabledChange={(value)=>void app.setReminderPreference(value)} onTestReminder={()=>{void Notifications.scheduleNotificationAsync({content:{title:'OweMate test reminder',body:'Notifications are working on this phone.',data:{kind:'test'}},trigger:{type:Notifications.SchedulableTriggerInputTypes.TIME_INTERVAL,seconds:5,repeats:false}})}} onBack={()=>app.setScreen('dashboard')}/></AppShell>;
