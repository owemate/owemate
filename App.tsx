...
type AppScreen = 'welcome' | 'signin' | 'signup' | 'forgotPassword' | 'dashboard' | 'add' | 'people' | 'personDetails';
...
function AppContent() {
  ...
  useEffect(() => {
    const subscription = BackHandler.addEventListener('hardwareBackPress', () => {
      const previousScreen: Partial<Record<AppScreen, AppScreen>> = { signin:'welcome', signup:'welcome', forgotPassword:'signin', add: app.addReturnScreen === 'personDetails' ? 'personDetails' : 'dashboard', people:'dashboard', personDetails:'people' };
      const previous = previousScreen[app.screen as AppScreen];
      if (!previous) return false;
      app.clearMessage(); app.setScreen(previous); return true;
    });
    return () => subscription.remove();
  }, [app.screen, app.addReturnScreen, app.clearMessage, app.setScreen]);
