...
type AppScreen = 'welcome' | 'signin' | 'signup' | 'dashboard' | 'add' | 'people' | 'personDetails';
...
function AppContent() {
  ...
  useEffect(() => {
    const subscription = BackHandler.addEventListener('hardwareBackPress', () => {
      const previousScreen: Partial<Record<AppScreen, AppScreen>> = { signin:'welcome', signup:'welcome', add:'dashboard', people:'dashboard', personDetails:'people' };
      const previous = previousScreen[app.screen as AppScreen];
      if (!previous) return false;
      app.clearMessage(); app.setScreen(previous); return true;
    });
  ...
  if (app.screen === 'personDetails' && personScreen) return <AppShell><PersonDetailsScreen person={personScreen} transactions={app.transactions} onBack={() => app.setScreen('people')} onAdd={() => app.setScreen('add')} onToggleSettled={(transaction) => void app.handleToggleSettled(transaction)} /></AppShell>;
