...
type Props = { transactions: Transaction[]; onBack: () => void; onSelectPerson: (person: string) => void };

export function PeopleScreen({ transactions, onBack, onSelectPerson }: Props) {
  const [query, setQuery] = useState('');
  const people = useMemo(() => { const normalized = query.trim().toLowerCase(); return buildPeopleSummary(transactions).filter((person) => !normalized || person.person.toLowerCase().includes(normalized)); }, [transactions, query]);
  const totalPeople = buildPeopleSummary(transactions).length;
  return <ScrollView style={styles.scroll} contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
    <Pressable onPress={onBack} hitSlop={10}><Text style={styles.back}>‹  Dashboard</Text></Pressable>
    <View style={styles.titleRow}><View><Text style={styles.eyebrow}>CONTACTS</Text><Text style={styles.title}>People</Text><Text style={styles.subtitle}>See who owes you and who you owe.</Text></View><View style={styles.count}><Text style={styles.countText}>{totalPeople}</Text></View></View>
    ...
    {people.map((person) => <Pressable style={styles.card} key={person.person} onPress={() => onSelectPerson(person.person)} android_ripple={{ color: '#EAF5F2' }}>
      ...
      <View style={styles.balanceBlock}><Text style={[styles.balance, person.balance < 0 && styles.negative]}>{person.balance >= 0 ? '+' : '-'}{formatCurrency(person.balance)}</Text><Text style={styles.balanceLabel}>{person.balance > 0 ? 'They owe you' : person.balance < 0 ? 'You owe' : 'Settled'}</Text><Text style={styles.viewLabel}>View ›</Text></View>
    </Pressable>)}
