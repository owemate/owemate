...
export function useAppRootState() {
  ...
  useEffect(() => {
    const loadReminderPreference = async () => {
      try {
        ...
        setRemindersEnabled(value !== 'false');
      } finally {
  ...
  useEffect(() => {
    if (!userId || !reminderPreferenceLoaded) {
      if (!userId) {
        setTransactions([]);
        setLoading(false);
      }
    ...
    const loadTransactions = async () => {
      ...
      try {
        const loaded = await fetchCloudTransactions(userId);
        if (!active) return;

        setTransactions(loaded);
        void syncTransactionReminders(loaded, remindersEnabled).catch(() => undefined);
      } catch (error) {
        if (!active) return;
        ...
        setMessage({
          type: 'error',
          text: getSupabaseErrorMessage(error, 'Could not load your records.'),
        });
      } finally {
  ...
  }, [userId, reminderPreferenceLoaded, remindersEnabled]);
  ...
  const handleAuth = async () => {
    ...
    if (!isSupabaseConfigured) {
      setMessage({
        type: 'error',
        text: 'Supabase is not configured in this local environment.',
      });
      return;
    }
    ...
    try {
      ...
      setMessage({
        type: 'error',
        text: getSupabaseErrorMessage(error, 'Authentication failed.'),
      });
    } finally {
  ...
  const handleForgotPassword = async () => {
    ...
    try {
      ...
      setMessage({
        type: 'error',
        text: getSupabaseErrorMessage(error, 'Could not send the reset link.'),
      });
    } finally {
      setAuthSubmitting(false);
    }
  ...
  const handleUpdatePassword = async () => {
    ...
    try {
      ...
      setMessage({
        type: 'error',
        text: getSupabaseErrorMessage(error, 'Could not update your password.'),
      });
    } finally {
      setAuthSubmitting(false);
    }
  ...
  const handleSaveTransaction = async () => {
    ...
    if (!userId) {
      setMessage({
        type: 'error',
        text: 'Please sign in before adding a money record.',
      });
      setScreen('signin');
      return;
    }
    ...
    if (cleanDueDate && !isValidUserDate(cleanDueDate)) {
      setMessage({
        type: 'error',
        text: 'Enter a valid commitment date, for example 28 Aug 2026.',
      });
      return;
    }
    ...
    } satisfies Omit<Transaction, 'id' | 'createdAt' | 'status'>;
    ...
    try {
      ...
      void syncTransactionReminders(
        nextTransactions,
        remindersEnabled,
      ).catch(() => undefined);

      const destination = addReturnScreen;
      resetEntry();
      setMessage({ type: 'success', text: 'Money record saved.' });
      setScreen(destination);
      setAddReturnScreen('dashboard');
    } catch (error) {
      setMessage({
        type: 'error',
        text: getSupabaseErrorMessage(error, 'Could not save the record.'),
      });
    } finally {
      setSaving(false);
    }
  ...
  const handleToggleSettled = async (transaction: Transaction) => {
    ...
    const nextStatus: TransactionStatus =
      transaction.status === 'settled' ? 'pending' : 'settled';
    ...
    try {
      ...
      setMessage({
        ...
        text:
          nextStatus === 'settled'
            ? 'Record marked as repaid.'
            : 'Record reopened.',
      });
    } catch (error) {
      setMessage({
        type: 'error',
        text: getSupabaseErrorMessage(error, 'Could not update the record.'),
      });
    }
  };
  ...
  const handleDeleteTransaction = async (transactionId: string) => {
    ...
    try {
      ...
      setMessage({
        type: 'error',
        text: getSupabaseErrorMessage(error, 'Could not delete the record.'),
      });
    }
  };
  ...
  const setReminderPreference = async (enabled: boolean) => {
    setRemindersEnabled(enabled);
    await AsyncStorage.setItem(REMINDERS_KEY, String(enabled)).catch(() => undefined);

    if (!enabled) {
      void syncTransactionReminders(transactions, false).catch(() => undefined);
    ...
    if (!granted) {
      ...
      await AsyncStorage.setItem(REMINDERS_KEY, 'false').catch(() => undefined);
      setMessage({
        type: 'error',
    ...
    void syncTransactionReminders(transactions, true).catch(() => undefined);
    setMessage({ type: 'success', text: 'Reminders turned on.' });
  };
