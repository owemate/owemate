...
type Props = { enabled: boolean; onEnabledChange: (value: boolean) => void; onTestReminder: () => void; onBack: () => void; };

export function ReminderSettingsScreen({ enabled, onEnabledChange, onTestReminder, onBack }: Props) {
  return <ScrollView style={styles.scroll} contentContainerStyle={styles.container}>
    ...
    <View style={styles.testCard}><Text style={styles.testTitle}>Test notifications</Text><Text style={styles.testText}>Send a test reminder in a few seconds to confirm notifications are working on this phone.</Text><Pressable disabled={!enabled} onPress={onTestReminder} style={[styles.testButton, !enabled && styles.disabled]}><Text style={styles.testButtonText}>Send test reminder</Text></Pressable></View>
